...
const CustomNavbar = () => {
  ...
  const [notifications, setNotifications] = useState([
    ...
    {
      id: 2,
      text: "กิจกรรม I² Starter Kit ใกล้เต็มแล้ว",
      link: "/events/2",
      read: false,
    },
    {
      id: 3,
      text: "ยินดีต้อนรับ! ขอบคุณที่สมัครเป็นสมาชิก",
      link: "/profile",
      read: true,
    },
    {
      id: 4,
      text: "สมาชิกของคุณจะหมดอายุในอีก 30 วัน",
      link: "/profile",
      read: true,
    },
  ]);
      ...
      title={
        isMobileView ? (
          <>
            <BellFill size={24} />
            {unreadCount > 0 && (
              <Badge pill bg="danger" className="notification-dot mobile">
                {unreadCount}
              </Badge>
            )}
          </>
        ...
        <>
          ...
          {user?.role === "admin" && (
            <Nav.Link
              as={NavLink}
              to="/admin/dashboard"
              className="mobile-user-link admin-link-mobile"
            >
              <Speedometer2 className="me-2" /> เมนูผู้ดูแลระบบ
            </Nav.Link>
          )}
          <Nav.Link as={NavLink} to="/profile" className="mobile-user-link">
            <PersonBadge className="me-2" /> โปรไฟล์
          </Nav.Link>
          <Nav.Link as={NavLink} to="/my-events" className="mobile-user-link">
            <Calendar2Check className="me-2" /> กิจกรรมของฉัน
          </Nav.Link>
          <hr className="mobile-menu-divider" />
          <Nav.Link
            onClick={handleLogout}
            className="mobile-user-link text-danger"
          >
            <BoxArrowRight className="me-2" /> ออกจากระบบ
          </Nav.Link>
        </>
      );
    }
    ...
    return (
        ...
        title={
          <div className="d-flex align-items-center">
            <div className="me-2">
              ...
              {user.member_type && (
                <Badge bg="info" className="ms-2 membership-badge">
                  {user.member_type}
                </Badge>
              )}
            </div>
      ...
      >
        <NavDropdown.Header>
          ...
          <div className="mt-1">
            <Badge bg={user.is_verified ? "success" : "warning"} className="me-1">
              {user.is_verified ? "ยืนยันแล้ว" : "ยังไม่ยืนยัน"}
            </Badge>
            <Badge bg="primary">{user.membership_type}</Badge>
          </div>
        </NavDropdown.Header>
        <NavDropdown.Item as={NavLink} to="/profile" end>
          <PersonBadge className="me-2" /> โปรไฟล์
        </NavDropdown.Item>
        <NavDropdown.Item as={NavLink} to="/my-events" end>
          <Calendar2Check className="me-2" /> กิจกรรมของฉัน
        </NavDropdown.Item>
        {user?.role === "admin" && (
          <>
                ...
                <div className="d-none d-lg-flex align-items-center">
                  ...
                  {user?.role === "admin" && (
                    <Nav.Link
                      as={NavLink}
                      to="/admin/dashboard"
                      className="btn-admin-panel me-2"
                    >
